...
/* device interface */
void* add_mmio_map(hwaddr_t addr, int len, mmio_callback_t callback) {
...
uint32_t mmio_read(hwaddr_t addr, int len, int map_NO) {
	assert(len >= 1 && len <= 4);
...
void mmio_write(hwaddr_t addr, int len, uint32_t data, int map_NO) {
	assert(len >= 1 && len <= 4);
	MMIO_t *map = &maps[map_NO];
